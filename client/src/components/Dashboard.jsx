...
export default function Dashboard() {
  ...
  // Digital Profile Builder States
  const [profileName, setProfileName] = useState('Sophia Martinez');
  const [profileTitle, setProfileTitle] = useState('Creative Director');
  const [profileCompany, setProfileCompany] = useState('Zenith Design');
  const [profileBio, setProfileBio] = useState('Building next-generation physical-to-digital branding experiences.');
  const [profileEmail, setProfileEmail] = useState('sophia@zenith.design');
  const [profilePhone, setProfilePhone] = useState('+1 (555) 938-2041');
  const [profileWebsite, setProfileWebsite] = useState('www.zenith.design');
  const [profileLocation, setProfileLocation] = useState('San Francisco, CA');
  const [profileAddress, setProfileAddress] = useState('100 Pine Street, Suite 2400\nSan Francisco, CA 94111');

  // Social Links States
  const [socialFacebook, setSocialFacebook] = useState('https://facebook.com/zenith.design');
  const [socialGoogle, setSocialGoogle] = useState('https://g.page/r/zenith.design/review');
  const [socialInstagram, setSocialInstagram] = useState('https://instagram.com/zenith.design');
  const [socialYoutube, setSocialYoutube] = useState('');
  const [socialLinkedin, setSocialLinkedin] = useState('https://linkedin.com/company/zenith-design');
  const [socialX, setSocialX] = useState('');

  // Dynamic Custom Links States
  const [customLinks, setCustomLinks] = useState([
    { id: 1, label: 'Portfolio Showcase', url: 'https://zenith.design/portfolio' }
  ]);
    ...
    <div className="min-h-screen bg-[#030712] pt-28 pb-16 px-4 md:px-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-8 glass border border-white/10 rounded-3xl relative overflow-hidden">
          ...
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-white mt-2">
              Welcome back, <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-slate-100 to-blue-400">{currentUser ? currentUser.phone : 'Member'}</span>
            </h1>
            <p className="text-slate-400 text-xs sm:text-sm mt-1 leading-relaxed">
              Manage your dynamic links, select visual card themes, and watch live mobile previews update instantly.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Left Column (Forms & Theme Configs) - Span 7 */}
            ...
            <div className="p-6 md:p-8 glass border border-white/5 rounded-3xl space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-white/5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-white text-base">Dynamic QR Generator</h3>
                    <span className="text-[10px] text-slate-400">Update destinations and color highlights</span>
                  </div>
                </div>
                <span className="text-[9px] font-bold text-blue-400 bg-blue-500/10 border border-blue-500/25 px-2 py-0.5 rounded-full uppercase tracking-wider">
                  Real-time
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-7 space-y-5">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Destination URL</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">QR Code Color</label>
                    <div className="flex items-center gap-3">
                      <input 
                        type="color" 
                        value={`#${qrColor}`}
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="flex-1 py-2.5 px-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20 text-white text-xs font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      {isCopied ? (
                        <>
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 px-4 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-bold transition-all flex items-center justify-center gap-1.5 border border-white/10 cursor-pointer hover:from-blue-500 hover:to-indigo-500"
                    >
                ...
                <div className="md:col-span-5 flex flex-col items-center justify-center p-4 bg-slate-950/40 border border-white/5 rounded-2xl relative overflow-hidden group">
                  ...
                  <span className="text-[8px] text-slate-500 font-extrabold uppercase tracking-widest mt-3">Live Active Preview</span>
                </div>
              </div>
            </div>

            {/* 2. Digital Profile Builder */}
            <div className="p-6 md:p-8 glass border border-white/5 rounded-3xl space-y-6">
              ...
              <div className="pb-6 border-b border-white/5 space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-white text-base">Digital Profile Builder</h3>
                    <span className="text-[10px] text-slate-400">Select active themes and enter contact info</span>
                  </div>
                </div>

                {/* Dynamic Theme Circular Selector */}
                <div className="space-y-2 pt-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Profile Color Theme</label>
                  <div className="flex flex-wrap items-center gap-4">
                    {themes.map((theme) => (
                      <button
                        key={theme.id}
                        onClick={() => setSelectedTheme(theme.id)}
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border border-white/5 transition-all text-xs font-bold text-slate-300 hover:bg-white/5 cursor-pointer ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Company / Business</label>
                  <input 
                    type="text"
                    value={profileCompany}
                    onChange={(e) => setProfileCompany(e.target.value)}
                    placeholder="e.g. Zenith Design"
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40"
                  />
                </div>
                ...
                {/* Multiline Address Input (Textarea) */}
                <div className="space-y-2 md:col-span-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Physical Address (Multiline)</label>
                  <textarea 
                    value={profileAddress}
                    onChange={(e) => setProfileAddress(e.target.value)}
                    rows={3}
                    placeholder="e.g. 100 Pine Street, Suite 2400&#10;San Francisco, CA 94111"
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40 resize-none leading-relaxed"
                  />
                </div>
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <textarea 
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40 resize-none leading-normal"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      placeholder="e.g. contact@domain.co"
                      className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4 md:col-span-2 pt-4 border-t border-white/5">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Social Network URLs</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40"
                      />
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] font-bold text-slate-500 uppercase flex items-center gap-1.5">
                      ...
                      <input 
                        ...
                        placeholder="https://g.page/r/yourid/review" 
                        className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40"
                      />
                    </div>
                    <div className="space-y-1">
                ...
                <div className="space-y-4 md:col-span-2 pt-4 border-t border-white/5">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Custom Links (Dynamic Panels)</label>
                    <button
                      type="button"
                      onClick={addCustomLink}
                      className="px-2.5 py-1.5 bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/20 text-blue-400 rounded-lg text-[10px] font-bold flex items-center gap-1 transition-all cursor-pointer"
                    >
                    ...
                    {customLinks.map((link) => (
                      <div key={link.id} className="p-4 bg-white/5 border border-white/5 rounded-2xl flex flex-col sm:flex-row items-center gap-3 relative group">
                        ...
                        <div className="w-full sm:w-[40%] space-y-1">
                          <span className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">Button Label</span>
                          <input 
                            type="text"
                            value={link.label}
                        ...
                        <div className="w-full sm:w-[60%] space-y-1">
                          <span className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">Destination URL</span>
                          <input 
                            type="url"
                            value={link.url}
                            onChange={(e) => updateCustomLink(link.id, 'url', e.target.value)}
                            placeholder="https://mystore.co"
                            className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-white text-xs focus:outline-none focus:border-blue-500/40"
                          />
                        </div>
                      </div>
          ...
          {/* Right Column (Live Phone Mockup Preview & NFC Cards) - Span 5 */}
          <div className="lg:col-span-5 space-y-8">
            
            {/* 1. Live Mobile Simulator Preview Mockup */}
            <div className="p-6 md:p-8 glass border border-white/5 rounded-3xl flex flex-col items-center">
