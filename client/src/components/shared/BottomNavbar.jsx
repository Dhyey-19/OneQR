...
export default function BottomNavbar() {
  ...
  const handleTabClick = (tab) => {
    if (tab.name === 'Home' && location.pathname === '/dashboard') {
      window.dispatchEvent(new Event('reset-dashboard-view'));
    } else {
      navigate(tab.path);
    }
  };
  ...
  return (
    <motion.div 
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white backdrop-blur-xl border-t border-slate-200/50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] pb-safe"
    >
        ...
        {tabs.map((tab) => {
          ...
          return (
            <motion.button
              ...
              className={`flex flex-col items-center justify-center w-14 h-12 rounded-2xl relative transition-all cursor-pointer ${
                isActive 
