...
export default function ConnectStandyModal({
  isOpen,
  onClose,
  profileId,
  onSuccess,
  ...
  const handleConnect = async (inputVal) => {
    ...
    try {
      ...
      setConnectStatus("error");
      setConnectMessage(
        err.message || "An error occurred while connecting the Standy.",
      );
    }
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="w-full max-w-md bg-white  border border-slate-200  rounded-3xl p-6 shadow-2xl relative overflow-hidden space-y-6 text-slate-900 "
          >
            {/* Header */}
            ...
            <div className="space-y-4">
              ...
              {connectStatus !== "loading" && connectStatus !== "success" && (
                <>
                    ...
                    <div className="space-y-4 text-left">
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3 rounded-xl border border-slate-200  bg-slate-50  focus:bg-white  text-slate-900  text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 transition-all placeholder:text-slate-400 "
                        />
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-3 rounded-xl bg-indigo-600 text-white font-bold text-xs hover:bg-indigo-500 hover:shadow-lg hover:shadow-indigo-500/20 transition-all border border-transparent  flex items-center justify-center gap-2 cursor-pointer shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                  ...
                  {/* Error State Banner */}
                  {connectStatus === "error" && (
                    <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-500  rounded-xl text-center text-xs font-bold leading-normal">
                      {connectMessage}
                    </div>
                  )}
                </>
