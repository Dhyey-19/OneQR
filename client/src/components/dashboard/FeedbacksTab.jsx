...
import { 
  Star, MessageSquare, Phone, User, Calendar, Inbox, 
  ArrowLeft, ShieldAlert, Sparkles, Heart, ThumbsUp, AlertTriangle
} from 'lucide-react';
  ...
  switch (rating) {
    case 5:
      return {
        ...
        cardBorderClass: 'border-emerald-500/15 dark:border-emerald-500/10 hover:border-emerald-500/30',
    ...
    case 4:
      return {
        ...
        cardBorderClass: 'border-blue-500/15 dark:border-blue-500/10 hover:border-blue-500/30',
    ...
    case 3:
      return {
        ...
        cardBorderClass: 'border-amber-500/15 dark:border-amber-500/10 hover:border-amber-500/30',
    ...
    case 2:
      return {
        ...
        cardBorderClass: 'border-orange-500/15 dark:border-orange-500/10 hover:border-orange-500/30',
    ...
    default:
      return {
        ...
        cardBorderClass: 'border-rose-500/15 dark:border-rose-500/10 hover:border-rose-500/30',
...
export default function FeedbacksTab() {
  ...
  feedbacks.forEach(f => {
    if (f.profile) {
      ...
      const bName = f.profile.profileCompany || f.profile.profileName || f.profile.slug;
  ...
  const filteredFeedbacks = feedbacks.filter(f => {
    const matchesStar = activeFilter === 'all' || f.rating === Number(activeFilter);
          ...
          <div className="p-6 glass border border-slate-200 dark:border-white/5 rounded-2xl shadow-sm hover:border-slate-300 dark:hover:border-white/10 transition-all flex flex-col justify-between">
            ...
            <div className="space-y-1 mt-3">
              {[5, 4, 3, 2, 1].map((stars) => {
                ...
                return (
                  <div key={stars} className="flex items-center gap-2 text-[10px] font-extrabold">
                    ...
                    <span className="w-5 text-right text-slate-500 dark:text-slate-400">{count}</span>
                  </div>
                );
              })}
            </div>
      ...
      <div className="glass border border-slate-200 dark:border-white/10 rounded-2xl md:rounded-3xl p-4 md:p-8 space-y-6 relative overflow-hidden">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            {/* Quick Star Filters */}
            <div className="flex flex-wrap gap-1.5 bg-slate-100 dark:bg-slate-900/60 p-1.5 border border-slate-200 dark:border-white/5 rounded-xl shrink-0">
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black tracking-wider uppercase transition-all cursor-pointer ${
                      isActive 
                        ? `${activeBg} shadow-sm` 
                        : 'text-slate-555 dark:text-slate-400 hover:text-slate-955 dark:hover:text-white'
                    }`}
                  >
                    {label}
                  </button>
          ...
          {uniqueBusinesses.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-150 dark:border-white/5">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-450 dark:text-slate-500 mr-2">
                Filter by Business:
              </span>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-black tracking-wider uppercase transition-all cursor-pointer ${
                    activeBusinessFilter === 'all'
                      ...
                      : 'bg-slate-100 dark:bg-slate-900/60 text-slate-550 dark:text-slate-400 border border-slate-200 dark:border-white/5 hover:text-slate-950 dark:hover:text-white'
                  }`}
                >
                  All Businesses
                </button>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black tracking-wider uppercase transition-all cursor-pointer ${
                      activeBusinessFilter === biz.id
                        ? 'bg-blue-600 text-white shadow-sm'
                        : 'bg-slate-100 dark:bg-slate-900/60 text-slate-555 dark:text-slate-400 border border-slate-200 dark:border-white/5 hover:text-slate-955 dark:hover:text-white'
                    }`}
                  >
                    {biz.name}
                  </button>
        ...
        {loading ? (
          ...
          <div className="border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden bg-white dark:bg-slate-900/10 divide-y divide-slate-200 dark:divide-white/5 shadow-sm">
            {filteredFeedbacks.map((f) => {
              const rStyle = getRatingStyle(f.rating);
              return (
                <div
                  key={f._id}
                  className="p-4 hover:bg-slate-50/50 dark:hover:bg-white/5 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 relative"
                >
                  {/* Left Side: Rating Stars, Business & Client Info */}
                  <div className="flex flex-wrap items-center gap-3 shrink-0">
                    {/* Rating Stars */}
                    <div className="flex items-center gap-0.5 bg-slate-100 dark:bg-slate-900/60 px-2 py-0.5 rounded-lg border border-slate-200 dark:border-white/5">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-3.5 h-3.5 ${
                  ...
                  <div className="flex-grow text-xs md:text-sm text-slate-700 dark:text-slate-300 font-medium md:px-4 break-words text-left">
                    ...
                    {f.customerPhone && (
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          ...
                          className="px-2.5 py-1 bg-green-500/10 border border-green-500/25 hover:bg-green-500/20 text-green-600 dark:text-green-400 rounded-lg font-black text-[9px] uppercase tracking-wide transition-colors cursor-pointer"
                        >
                          Call
                        </a>
                      </div>
