...
import { Check, Link2, Download, Sparkles, Smartphone, ShieldAlert, ExternalLink, X } from 'lucide-react';
import { downloadFlyer, downloadDynamicFlyer } from '../../utils/flyerDownloader';
import { FaInstagram, FaFacebook, FaWhatsapp, FaYoutube, FaLinkedin, FaTwitter } from 'react-icons/fa';
...
export default function AllocatedQrCard({ profile, onManage, onConnect }) {
  ...
  const formatSocialUrl = (platform, value) => {
    ...
    switch (platform) {
      ...
      case 'x': return `https://twitter.com/${value.replace('@', '')}`;
      case 'linkedin': return `https://linkedin.com/in/${value}`;
      case 'youtube': return `https://youtube.com/${value.startsWith('@') ? '' : '@'}${value.replace('@', '')}`;
  ...
  const availableQRs = [
    { id: 'all', name: 'All In One QR', url: finalQrUrl, icon: Sparkles, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-500/10', theme: null }
  ];
  if (profile.socialGoogle) availableQRs.push({ id: 'google', name: 'Google Review QR', url: formatSocialUrl('google', profile.socialGoogle), icon: FcGoogle, color: 'text-black', bg: 'bg-slate-100 dark:bg-white/10', isTemplate: true, templateUrl: '/assets/google_review.png' });
  if (profile.socialInstagram) availableQRs.push({ id: 'instagram', name: 'Instagram QR', url: formatSocialUrl('instagram', profile.socialInstagram), icon: FaInstagram, color: 'text-pink-600', bg: 'bg-pink-50 dark:bg-pink-500/10', theme: { name: 'Instagram', logoSvgUrl: instagramLogoSvg, gradientColors: ['#833AB4', '#FD1D1D', '#F56040'], actionText: 'Follow us on' }});
  if (profile.socialFacebook) availableQRs.push({ id: 'facebook', name: 'Facebook QR', url: formatSocialUrl('facebook', profile.socialFacebook), icon: FaFacebook, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-500/10', theme: { name: 'Facebook', logoSvgUrl: facebookLogoSvg, bgColor: '#1877F2', actionText: 'Connect on' }});
  if (profile.socialWhatsapp) availableQRs.push({ id: 'whatsapp', name: 'WhatsApp QR', url: formatSocialUrl('whatsapp', profile.socialWhatsapp), icon: FaWhatsapp, color: 'text-green-500', bg: 'bg-green-50 dark:bg-green-500/10', theme: { name: 'WhatsApp', logoSvgUrl: whatsappLogoSvg, bgColor: '#25D366', actionText: 'Message us on' }});
  if (profile.socialYoutube) availableQRs.push({ id: 'youtube', name: 'YouTube QR', url: formatSocialUrl('youtube', profile.socialYoutube), icon: FaYoutube, color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-500/10', theme: { name: 'YouTube', logoSvgUrl: youtubeLogoSvg, bgColor: '#FF0000', actionText: 'Subscribe on' }});
  if (profile.socialLinkedin) availableQRs.push({ id: 'linkedin', name: 'LinkedIn QR', url: formatSocialUrl('linkedin', profile.socialLinkedin), icon: FaLinkedin, color: 'text-blue-700', bg: 'bg-blue-50 dark:bg-blue-500/10', theme: { name: 'LinkedIn', logoSvgUrl: linkedinLogoSvg, bgColor: '#0A66C2', actionText: 'Connect on' }});
  if (profile.socialX) availableQRs.push({ id: 'x', name: 'X (Twitter) QR', url: formatSocialUrl('x', profile.socialX), icon: FaTwitter, color: 'text-slate-900 dark:text-white', bg: 'bg-slate-200 dark:bg-white/10', theme: { name: 'X (Twitter)', logoSvgUrl: xLogoSvg, bgColor: '#000000', actionText: 'Follow us on' }});
  ...
  const handleDownloadItem = async (e, item) => {
    ...
    try {
      if (item.id === 'all' || item.isTemplate) {
        await downloadFlyer(item.url, profile.slug || qrId, profile.profileCompany, item.templateUrl);
      } else {
        await downloadDynamicFlyer(item.url, profile.profileCompany || profile.profileName, item.theme);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      ...
      {isDownloadModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" onClick={(e) => { e.stopPropagation(); setIsDownloadModalOpen(false); }}>
          <div 
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Download QR Code</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Select a layout to download as a 9:16 poster</p>
              </div>
              <button 
                onClick={(e) => { e.stopPropagation(); setIsDownloadModalOpen(false); }}
                className="w-8 h-8 rounded-full bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            
            <div className="p-3 max-h-[60vh] overflow-y-auto overscroll-contain" data-lenis-prevent="true">
                ...
                {availableQRs.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-4 w-full p-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-white/5 border border-transparent hover:border-slate-200 dark:hover:border-white/10 transition-all group cursor-pointer text-left"
                    >
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${item.bg}`}>
                        <Icon className={`w-6 h-6 ${item.color}`} />
                      </div>
                      <div className="flex-1">
                        <span className="block text-sm font-bold text-slate-900 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">
                          {item.name}
                        </span>
                        <span className="block text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1 break-all pr-2">
                          {item.url}
                        </span>
                      </div>
                      <Download className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-brand-600 dark:group-hover:text-brand-400 shrink-0" />
                    </button>
                  );
                })}
              </div>
