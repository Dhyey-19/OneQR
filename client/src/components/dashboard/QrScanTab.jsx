...
export default function QrScanTab({ onSelectAndManageQr, onRefreshQrs }) {
  ...
  const parseQrIdFromScannedText = (text) => {
    ...
    try {
      ...
      if (
        cleanText.toLowerCase().startsWith("http://") ||
  ...
  const handlePageQrScan = async (inputVal) => {
    ...
    try {
      ...
      if (response.status === "assigned_now") {
        ...
        setPageScanMessage(
          response.message || "your qr is live and link with this property",
        );
        ...
        setPageScanMessage(
          response.message ||
            "This QR Code is already allocated to your workspace.",
        );
        ...
        response.status === "already_assigned_to_other" ||
        ...
        setPageScanStatus("error");
        setPageScanMessage(
          response.message || "An unexpected response occurred.",
        );
      }
    } catch (err) {
      ...
      setPageScanStatus("error");
      setPageScanMessage(
        err.message || "An error occurred while scanning the QR code.",
      );
    }
  };
      ...
      const timer = setTimeout(() => {
        ...
        html5QrcodeInstance
            ...
            (decodedText) => {
              ...
              if (html5QrcodeInstance && html5QrcodeInstance.isScanning) {
                html5QrcodeInstance
                  .stop()
                  .catch((err) =>
                    console.error("Error stopping page scanner:", err),
                  );
              }
            },
            (errorMessage) => {
          ...
          .catch((err) => {
            ...
            setPageCameraError(
              "Camera access denied or device is not available. Please verify permissions.",
            );
          });
      }, 300);
      ...
      return () => {
        ...
        if (html5QrcodeInstance && html5QrcodeInstance.isScanning) {
          html5QrcodeInstance
            ...
            .catch((err) =>
              console.error("Error clearing page scanner on unmount:", err),
            );
        }
      };
    }
    ...
    <div className="space-y-8 animate-fade-in max-w-xl mx-auto">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-xs sm:text-sm text-slate-500 ">
          Verify QR assignment status and link it to your dynamic property
          profiles.
        </p>
      </div>

      {/* Scanner Card */}
      <div className="p-6 glass border border-slate-200  rounded-3xl relative overflow-hidden space-y-6">
        {/* Tab Selector */}
        ...
        <div className="space-y-4">
          ...
          {pageScanStatus === "success" && (
            <div className="flex flex-col items-center justify-center py-12 space-y-4 text-center">
              ...
              <p className="text-xs text-slate-655  max-w-xs leading-relaxed font-bold">
                {pageScanMessage}
              </p>
            ...
            <>
              {pageScanTab === "scan" ? (
                <div className="space-y-5">
                  <div className="relative w-full aspect-square max-w-[280px] mx-auto overflow-hidden rounded-2xl border border-slate-200  bg-black flex items-center justify-center">
                    <div
                      id="page-qr-reader-container"
                  ...
                  {pageCameraError ? (
                    <div className="p-4 bg-slate-50  border border-slate-200  rounded-2xl text-xs space-y-3 leading-relaxed text-left font-semibold max-w-sm mx-auto">
                      <strong className="text-slate-700  font-extrabold block uppercase tracking-wider text-[10px] mb-1.5 text-center">
                      ...
                      <div className="space-y-2.5 text-slate-600 ">
                        <p className="flex items-start gap-2">
                          ...
                          <span>
                            Tap the **lock/settings icon** next to the URL in
                        ...
                        <p className="flex items-start gap-2">
                          ...
                          <span>
                            Locate the **Camera** option and switch it to
                            **Allow**.
                          </span>
                        </p>
                        <p className="flex items-start gap-2">
                    ...
                    <p className="text-[10px] text-slate-550  text-center leading-relaxed max-w-xs mx-auto font-semibold">
                      Allow camera access and point your camera at the OneQR
                      code printed/displayed.
                    </p>
                  )}
                </div>
              ) : (
                <div className="space-y-4 text-left">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-slate-200  bg-slate-50  focus:bg-white  text-slate-900  text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all placeholder:text-slate-400 "
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-xl bg-[#2563eb] text-white font-bold text-xs hover:bg-[#1d4ed8] hover:shadow-lg hover:shadow-blue-500/20 transition-all border border-transparent  flex items-center justify-center gap-2 cursor-pointer shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
                  >
              ...
              {pageScanStatus === "error" && (
                <div className="p-3.5 bg-red-500/10 border border-red-500/20 text-red-500  rounded-xl text-center text-xs font-bold leading-normal">
                  {pageScanMessage}
                </div>
              )}
            </>
