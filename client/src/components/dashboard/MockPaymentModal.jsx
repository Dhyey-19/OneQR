  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[999] flex items-center justify-center p-4">
      <motion.div
        ...
        className="w-full max-w-md bg-white  border border-slate-200  rounded-3xl p-6 shadow-2xl relative space-y-6 animate-fade-in"
      >
        {/* Header banner */}
        ...
        {/* Note text */}
        <div className="p-4 bg-amber-500/5 border border-amber-500/15 rounded-2xl text-xs text-slate-700  leading-relaxed space-y-2">
          <p>
            <strong>Notice:</strong> Your Razorpay credentials (
            <code>RAZORPAY_KEY_ID</code> and <code>RAZORPAY_KEY_SECRET</code>)
            are not yet configured in the server's <code>.env</code> file.
          </p>
          <p>
            The gateway has loaded this sandbox modal to allow you to simulate a
            successful checkout and verify your subscription workflow
            end-to-end.
          </p>
        </div>
        ...
        <div className="bg-slate-50  border border-slate-100  rounded-2xl p-4 space-y-3">
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="text-slate-900  font-extrabold">
              ₹{(mockPaymentData.amount / 100).toFixed(2)}
            </span>
          </div>
          <div className="flex justify-between items-center text-xs">
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-3 rounded-xl border border-slate-200  hover:bg-slate-100  text-slate-500  hover:text-slate-800  text-xs font-bold transition-all cursor-pointer"
          >
            Cancel Transaction
          </button>
          <button
