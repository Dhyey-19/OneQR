...
export default function FilePreview({ doc }) {
  ...
  const isImage = () => {
    ...
    return (
      ...
      /\.(jpeg|jpg|gif|png|webp|svg)/i.test(u)
    );
  };
  ...
  const isPdf = () => {
    ...
    const u = doc.url || "";
    return (
      name.toLowerCase().endsWith(".pdf") || u.toLowerCase().endsWith(".pdf")
    );
  };
  ...
  if (isImage()) {
    return (
      <div className="w-16 h-16 rounded-xl bg-slate-100  border border-slate-200  overflow-hidden shrink-0 relative group/thumb cursor-pointer">
        <img
          src={url}
          alt={doc.filename}
  ...
  if (isPdf()) {
    ...
    return (
      <div className="w-16 h-16 rounded-xl bg-white border border-slate-200  overflow-hidden shrink-0 relative group/thumb cursor-pointer">
        {thumbnailUrl ? (
          <img
            src={thumbnailUrl}
            alt={doc.filename}
            ...
            onError={(e) => {
              ...
              const iframeWrapper = e.target.parentElement.querySelector(
                ".pdf-iframe-wrapper",
              );
        ...
        <div
          className={`pdf-iframe-wrapper ${thumbnailUrl ? "hidden" : "block"} absolute inset-0 pointer-events-none overflow-hidden bg-white z-0`}
        >
          <iframe
  ...
  return (
    <div className="w-16 h-16 rounded-xl bg-slate-100  border border-slate-200  flex flex-col items-center justify-center text-slate-500  shrink-0 relative group/thumb cursor-pointer">
      <span className="text-[9px] font-bold tracking-wider uppercase mb-1 text-slate-500">
        FILE
      </span>
      <a
        href={url}
