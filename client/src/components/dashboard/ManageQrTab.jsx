              ...
              <div className="flex-1 flex flex-col overflow-y-auto relative no-scrollbar pb-6">
                ...
                <div className="px-4 flex flex-col flex-1 relative z-10">
                  <div className="flex flex-col items-center -mt-8 mb-0 relative z-20">
                    ...
                    {profileAddress && (
                      <div className="flex flex-col items-center justify-center mt-1 text-slate-500">
                        ...
                        {profileMapUrl && (
                          <a 
                            ...
                            className="mt-1 px-2 py-0.5 bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/20 text-blue-500 rounded text-[7px] font-black transition-all flex items-center gap-0.5"
                          >
                    ...
                    {profileTimings && (
                      <div className="flex items-center justify-center gap-1 mt-1 text-slate-500">
                        ...
                        <span className="text-[8px] font-bold truncate max-w-[150px]">{profileTimings}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-4 mb-2">
                    {(() => {
                      const actionCards = [];
                      if (profilePhone) {
                        actionCards.push({ id: 'call', icon: Phone, color: 'text-green-500', label: profilePhone, href: `tel:${profilePhone}`, target: undefined });
                        actionCards.push({ id: 'save', icon: User, color: 'text-indigo-500', label: 'Save Contact', href: '#', target: undefined });
                      }
                      if (profileEmail) {
                        actionCards.push({ id: 'email', icon: Mail, color: 'text-yellow-500', label: profileEmail, href: `mailto:${profileEmail}`, target: undefined });
                      }
                      if (profileWebsite) {
                        const targetUrl = /^https?:\/\//i.test(profileWebsite) ? profileWebsite : `https://${profileWebsite}`;
                        actionCards.push({ id: 'web', icon: Globe, color: 'text-blue-500', label: profileWebsite, href: targetUrl, target: '_blank' });
                      }
                          ...
                          {actionCards.map((card, idx) => {
                            ...
                            return (
                              <a 
                                ...
                                className={`w-full py-2 px-2.5 rounded-xl flex items-center justify-between text-[9px] font-bold ${activeTheme.itemBg} ${isLastOdd ? 'col-span-2' : ''} hover:scale-[1.01] transition-transform`}
                              >
                          ...
                          {socialOrder.map(key => {
                            ...
                            return (
                              <a 
                                ...
                                className={`py-1.5 px-2 rounded-xl flex items-center gap-1.5 text-[8px] font-bold ${activeTheme.buttonBg} hover:scale-[1.02] transition-all`}
                              >
                                {p.imgSrc ? (
